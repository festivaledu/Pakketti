...
router.get("/hero", async (req, res) => {
	...
	if (!packageObj.headerImage || !packageObj.headerImageMime) return res.status(httpStatus.NOT_FOUND).send({
		error: {
			name: httpStatus[httpStatus.NOT_FOUND],
			code: httpStatus.NOT_FOUND,
			message: `Package \"${packageObj.identifier}\" does not contain any hero image`
		}
	});

	res.header("Content-Type", packageObj.headerImageMime);
	res.write(packageObj.headerImage, "binary");
	return res.end(undefined, "binary");
});
 ...
 * PUT /packages/hero
 * 
 * Updates the hero image of a specified Package
 */
router.put("/hero", async (req, res) => {
	...
	let headerImageFile = req.files.file;
	if (!headerImageFile) return res.status(httpStatus.BAD_REQUEST).send({
		error: {
			name: httpStatus[httpStatus.BAD_REQUEST],
			code: httpStatus.BAD_REQUEST,
			message: "No hero image file specified"
		}
	});
	
	return packageObj.update({
		headerImage: headerImageFile.data,
		headerImageMime: headerImageFile.mimetype
	}).then(() => {
		LogItem.create({
			id: String.prototype.concat(new Date().getTime, Math.random()),
			type: LogItemType.PACKAGE_EDITED,
			accountId: account.id,
			affectedPackageId: packageObj.id,
			detailText: `Package ${packageObj.identifier} <${packageObj.id}> received a new hero image`,
			status: LogItemStatus.LOG_USAGE
		});
		...
		return res.status(httpStatus.OK).send({
			success: {
				name: httpStatus[httpStatus.OK],
				code: httpStatus.OK,
				message: "Hero image successfully updated"
			}
		});
	}).catch(error => ErrorHandler(req, res, error));
});
...
/**
 * DELETE /packages/hero
 * 
 * Deletes the hero image associated to a Package
 */
router.delete("/hero", async (req, res) => {
	...
	return packageObj.update({
		...
		LogItem.create({
			...
			accountId: account.id,
			affectedPackageId: packageObj.id,
			detailText: `Package ${packageObj.identifier} <${packageObj.id}> received a null hero image`,
			status: LogItemStatus.LOG_USAGE
		});
		...
		return res.status(httpStatus.OK).send({
			success: {
				name: httpStatus[httpStatus.OK],
				code: httpStatus.OK,
				message: "Hero image successfully deleted"
			}
		});
	}).catch(error => ErrorHandler(req, res, error));
});
